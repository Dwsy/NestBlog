...
export class Comments{
    ...
    @ApiProperty({description:'评论者客户端',example:'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.96 Safari/537.36'})
    @prop()
    agent:string
    ...
    @ApiProperty({description:'评论状态',example:'approved（通过）|spam（垃圾箱）|waiting（审核中）'})
    @prop()
